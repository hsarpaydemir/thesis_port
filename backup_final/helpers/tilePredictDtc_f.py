...
def tilePredictDtc_f(data, opts):
    ...
    for num in range(data.shape[3]):
        ...
        for yi in range(ntiles_y):
            ...
            for xi in range(ntiles_x):
                ...
                pred_cls, pred_semseg = model(torch.permute(torch.tensor(paddedInputSlice).unsqueeze(0), (0, 3, 1, 2)).to(device), mock_gates)

                for c_i in range(len(opts['expectedPredData'])):
                    blobName = opts['expectedPredData'][c_i]
                    scoreSlice = pred_cls

                    if yi == 0 and xi == 0:
                        scores_tmp = np.zeros((data.shape[0], data.shape[1], scoreSlice.shape[1], scoreSlice.shape[0]), dtype=np.single)
                    else:
                        scores_tmp = scores_t[c_i]
                    scoreSlice = scoreSlice.cpu().detach().numpy()
                    if opts['average_mirror'] == True:
                        print('opts.average_mirror')
                        scores_torch, _ = model(torch.permute(torch.tensor(np.fliplr(paddedInputSlice).copy()).unsqueeze(0), (0, 3, 1, 2)).to(device), mock_gates)
                        scoreSlice += np.fliplr(scores_torch.cpu().detach().numpy())
                        scores_torch, _ = model(torch.permute(torch.tensor(np.flipud(paddedInputSlice).copy()).unsqueeze(0), (0, 3, 1, 2)).to(device), mock_gates)
                        scoreSlice += np.flipud(scores_torch.cpu().detach().numpy())
                        scores_torch, _ = model(torch.permute(torch.tensor(np.flipud(np.fliplr(paddedInputSlice)).copy()).unsqueeze(0), (0, 3, 1, 2)).to(device), mock_gates)
                        scoreSlice += np.flipud(np.fliplr(scores_torch.cpu().detach().numpy()))
                    ...
                    scores_tmp[xfromto[0]:xfromto[1], yfromto[0]:yfromto[1], :] = np.transpose(scoreSlice, (2, 3, 1, 0))[:validReg[0], :validReg[1], :]
                    scores_t[c_i] = scores_tmp
                    if debugMode > 0:
