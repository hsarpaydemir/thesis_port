...
class AugmentData_v2_f(object):
    ...
    def __call__(self, sData_c, sLabels_c, sClasses_c, origWeights_c, sfD_c):
        sData_c = np.array(torch.permute(sData_c, (2, 1, 0))) #Getting the tensor to the matlab shape
        sLabels_c = np.array(torch.permute(sLabels_c, (2, 1, 0))) #Getting the tensor to the matlab shape
        ...
        if self.has_smpl_dstrb:
            ...
            #sfD = ip.ev(xT, yT)
            sfD = self.interpn_python(sfD, xT, yT, method='nearest')
        ...
        #Apply to data
        augD_tmp = self.applyOpChannelWise(self.interpn_python, sData_c, self.sizeAugData_l[:3], xT, yT, method='nearest')
        ...
        if np.any(origWeights_c > 0):
            tmp_Weights = self.interpn_python(origWeights_c, xT, yT, method='nearest')
    ...
    def interpn_python(x, xT, yT, method='linear'):
        ...
        if method == 'linear':
            ...
            x = map_coordinates(x, [xT, yT], order=1, mode='nearest').reshape(508, 508)
        return x
