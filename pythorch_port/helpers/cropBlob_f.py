...
def cropBlob_f(data_c, netSize_l, isNetIn=False):
    data_c = sio.loadmat('cropblob.mat')['dskswCl']
...
def cropIt2NetS_loc(data, netSize_l):
    ...
    labCropRect = [(netSize_l[0], netSize_l[1]) / 2 + 1, (netSize_l[0], netSize_l[1]) / 2 + 1, (netSize_l[0], netSize_l[1]) / 2 + netSize_l[1], (netSize_l[0], netSize_l[1]) / 2 + netSize_l[1]]
    croppedData = np.zeros((netSize_l[1], netSize_l[1], data.shape[2], data.shape[3]), like=data)
...
def myImcrop(im, labCropRect):
    im = im[labCropRect[0]:labCropRect[2], labCropRect[1]:labCropRect[3]]
    return im