...
class DiskMaskNet(nn.Module):
    ...
    def forward(self, x):
        ...
        n1_u1d = self.first_part(n1_d1c)
        pred_cls = self.predict_ref_points(n1_u1d)

        #predict instance masks
        obj_rp = self.predict_instance_masks(n1_u1d)
        #Activate some ref points
        sem_scores_tld = torch.tile(gates, (1, 64, 1, 1)) #####Axis = 1 for caffe tile operation
        obj_rp_act = torch.mul(sem_scores_tld, obj_rp)

        #### crop 324 to in = 316 to get out = 132
        obj_rp_cr = obj_rp_act[:, :, 4:4 + DummyData_316.shape[2], 4:4 + DummyData_316.shape[3]] #Crop layer

        n2_d0b = self.n2_d0b(obj_rp_cr)
        concat_n2_d2a_u1d = torch.cat((n2_d0b, n1_d1c), dim=1)
        return pred_cls
