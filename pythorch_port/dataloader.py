...
class AugmentData_v2_f(object):
    ...
    def __call__(self, sData_c, sLabels_c, sfD_c):
        ...
        #Apply to data
        augD_tmp = self.applyOpChannelWise(self.interpn_python, sData_c, self.sizeAugData_l, xT, yT)
    ...
    #Helper functions
    def interpn_python(self, x, xT, yT):
        ip = SmoothBivariateSpline(range(x.shape[0]), range(x.shape[1]), x)
        x = ip.ev(xT, yT)
        return x

    def applyOpChannelWise(self, func, A, outShape, xT, yT):
        B = np.zeros(outShape, dtype=type(A))
        for ch_i in range(0, A.shape[2]):
            B[:, :, ch_i] = func(A[:, :, ch_i], xT, yT)
        return B

    def myImcrop(self, im, labCropRects_l):
