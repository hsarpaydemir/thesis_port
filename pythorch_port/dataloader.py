...
class AugmentData_v2_f(object):
    ...
    def __call__(self, sData_c, sLabels_c, sClasses_c, origWeights_c, sfD_c):
        ...
        #Apply to data
        augD_tmp = self.applyOpChannelWise(self.interpn_python, sData_c, self.sizeAugData_l[:3], xT, yT, method='linear')
...
for aim_i in range(NUMB_AUG_IMGS):
    ...
    augWeights_c.append(cropBlob_f(wghtCls, augment_opts['netSize_1_l']))
    augLabs_c2 = cropBlob_f(dskswCl, augment_opts['netSize_1_l'])
    augLabs_c3 = cropBlob_f(tmp_augLabs_c[aim_i], augment_opts['netSize_1_l'])
    augLabs_c.append([augLabs_c1, augLabs_c2, augLabs_c3])
    print(weightSpaceBtwDsks_f)
